function setup() {
    ...
    function draw() {
        ...
        // draw a cube
        function drawCube(color, TxU, scale, TlookAt) {
            ...
            // Face 1 -- along z-axis
            moveToTx([-1, -1, -1], Tx); lineToTx([1, -1, -1], Tx);
            lineToTx([1, 1, -1], Tx); lineToTx([-1, 1, -1], Tx);
            //lineToTx([-1,-1,-1],Tx);
            context.closePath();
            context.fill();
            ...
            // calcualte normal vector of face 3
            var v1 = vec3.fromValues(1, -1, -1);
            var v2 = vec3.fromValues(1, -1, 1);
            var v3 = vec3.fromValues(1, 1, 1);
            var normal = vec3.create();
            vec3.cross(normal, vec3.subtract(vec3.create(), v1, v2), vec3.subtract(vec3.create(), v2, v3));
            //vec3.normalize(normal, normal);
            console.log(normal);
            //draw3DAxes("purple", Tx, 10.0);
            // draw normal vector
            // context.beginPath();
            // context.strokeStyle = "black";
            // context.strokeWidth = 5;
            // moveToTx([-3, 0, 0], Tx); lineToTx(normal, Tx);
            // context.closePath();

            // if cos of angle between normal and TlookAt is positive, draw face 3
            if (vec3.dot(normal, TlookAt) > 0) {
                context.beginPath();
        ...
        drawCube("green", tVP_PROJ_VIEW_Camera, cube_scale, tVP_PROJ_VIEW_Camera); //NITZAN
        ...
        // transform off of cube
        var Tleg_to_cube_cam = mat4.create();
        mat4.translate(Tleg_to_cube_cam, tVP_PROJ_VIEW_Camera, [cube_scale, 0, 0]);
        drawBorder("red", Tleg_to_cube_cam, 100.0);
        //draw3DAxes("red", Tleg_to_cube_cam, 100.0);
        var Tleg_to_cube_cam2 = mat4.create();
        mat4.translate(Tleg_to_cube_cam2, tVP_PROJ_VIEW_Camera, [-cube_scale, 0, 0]);
        drawBorder("red", Tleg_to_cube_cam2, -100.0);
        //draw3DAxes("red", Tleg_to_cube_cam2, -100.0);
        ...
        drawCube2("green", tVP_PROJ_VIEW_Observer, cube_scale); //NITZAN

        // transform off of cube
        var Tleg_to_cube_obs = mat4.create();
        mat4.translate(Tleg_to_cube_obs, tVP_PROJ_VIEW_Observer, [cube_scale, 0, 0]);
        drawBorder("red", Tleg_to_cube_obs, 100.0);
        //draw3DAxes("red", Tleg_to_cube_obs, 100.0);
        var Tleg_to_cube_obs2 = mat4.create();
        mat4.translate(Tleg_to_cube_obs2, tVP_PROJ_VIEW_Observer, [-cube_scale, 0, 0]);
        drawBorder("red", Tleg_to_cube_obs2, -100.0);
